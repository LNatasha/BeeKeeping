...
struct FeedingHistoryView: View {
    ...
    private var alimentacoes: [Feeding] {
        store.hives.first { $0.id == hiveId }?.alimentacoes ?? []
    }
            ...
            VStack(spacing: 12) {
                if alimentacoes.isEmpty {
                    VStack(spacing: 10) {
                        ...
                        Text("Sem registos de alimentação")
                            .font(.system(size: 15))
                    ...
                    ForEach(alimentacoes) { f in
                        FeedingCard(feeding: f, hiveId: hiveId)
                    }
                }
            }
            .padding(16)

            NavigationLink(value: NavTarget.feedingForm(hiveId)) {
...
struct FeedingCard: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            HStack(alignment: .top) {
                VStack(alignment: .leading, spacing: 5) {
                    Text(feeding.data.ddMMYYYY)
                        .font(.system(size: 18, weight: .bold))
                        .foregroundStyle(Color.brownDark)
                    Text(feeding.produto)
                ...
                VStack(alignment: .trailing, spacing: 8) {
                    HStack(spacing: 4) {
                        Image(systemName: feeding.tipo.icon)
                            .font(.system(size: 10))
                        Text(feeding.tipo.rawValue)
                            .font(.system(size: 11, weight: .semibold))
                    }
                    .foregroundStyle(feeding.tipo.badgeColor)
                    .padding(.horizontal, 10)
                    .padding(.vertical, 4)
                    .background(feeding.tipo.badgeBackground)
                    .clipShape(Capsule())
            ...
            VStack(spacing: 7) {
                ...
                let trimmed = feeding.notas.trimmingCharacters(in: .whitespaces)
                if !trimmed.isEmpty {
                    Text(trimmed)
                        .font(.system(size: 12))
                        .italic()
        ...
        .sheet(isPresented: $showEdit) {
            EditFeedingView(feeding: feeding, hiveId: hiveId)
                .environment(store)
        }
    }
}
